...
exports.findById = (req, res) => {
  ...
  Product_attributes.findById(id, (err, result) => {
    if (err) {
      res.status(500).json({
        code: 500,
        status: 'INTERNAL SERVER ERROR',
...
exports.create = (req, res) => {
  try {
    ...
    const { products_id, products_size } = req.body;

    Product_attributes.create(products_id, products_size, (err, result) => {
      if (err) {
        res.status(500).json({
          code: 500,
          status: 'INTERNAL SERVER ERROR',
...
exports.update = (req, res) => {
  ...
  Product_attributes.update(id, products_id, products_size, (err, result) => {
    if (err) {
      res.status(500).json({
        code: 500,
        status: 'INTERNAL SERVER ERROR',
